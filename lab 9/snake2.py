...
def restart_game():
    ...
    big_fruit = randrange(0, res, size), randrange(0, res, size) 
    aaa =   pygame.image.load('food3.png')
    aa = pygame.transform.scale(aaa, (50,50))
    big_fruit_img = aa
    big_fruit_size = size * 2  
    big_fruit_score = 5  
    ...
    fo = pygame.image.load('fonsnake.jpg')
    ...
    while running:
        ...
        screen.blit(big_fruit_img, big_fruit)
        ...
        if snake[-1] == big_fruit:  # Check for collision with big fruit
            big_fruit = randrange(0, res, size), randrange(0, res, size)
            length += 5
            score += big_fruit_score
            fps += 1
